...
router.post("/create-ticket", async (req, res) => {
    try {
        ...
        const newTicket = await pool.query(
            ...
            [title, description, status, created, deadline, priority, submitter, projectID]
        );
...
router.get("/retrieve/paginated-project", async (req, res) => {
    try {
        ...
        res.json({tickets: paginatedTickets.rows, totalTicketCount: totalTicketCount.rows.length});
    } catch (err) {
        console.log(err.message);
    }
});
