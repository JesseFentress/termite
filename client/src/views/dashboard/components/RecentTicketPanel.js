...
import { getRecentTickets } from '../../../util/ticketHandler';
 ...
 export const RecentTicketPanel = ({
    token
 }) => {
                ...
                <table className="table table-sm table-hover w-100">
                    ...
                    { tickets.map((ticket, index) => (
                        <tr className="w-100 m-0 mt-2 center" role="button" key={index}>
                                <td className="col-1 align-middle"><div className={`dot p-2 bg-${priorities[ticket.priority]}`}></div></td>
                                ...
                                <td className="col-2"><div className={`overflow-hidden overflow-ellipsis text-capitalize c-${status[ticket.status]}`}>{ticket.status}</div></td>
                                <td className="col-2"><div className="overflow-hidden overflow-ellipsis">{formatDate(ticket.date_created)}</div></td>
                                <td className="col-2"><div className="overflow-hidden overflow-ellipsis">{formatDate(ticket.date_deadline)}</div></td>
                        </tr>
                    ))}                
                    </tbody>
                </table>
                :
                <div><p>No Ticket Information Found</p></div>
            }
        </Panel>
    );
 };
