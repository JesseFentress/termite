    ...
    return (
        <div className="mt-4">
            { potentialTeamMembers ?
                <form onSubmit={formik.handleSubmit}>
                    ...
                    { potentialTeamMembers.map((member, index) => (
                        <li key={index} className={`d-flex ${getBackgroundColor(index)}`}>
                            <div className="px-4">
                                <input type="checkbox" id={index} name="newTeamMemberIDs" value={member.id} onChange={formik.handleChange}/>
                                <label className="mx-2" htmlFor={index}>{member.first_name + " " + member.last_name}</label>
                            </div>
                        </li>
                    ))}
                    </ul>
                ...
                :
                <div><p>No New Team Member Information Found</p></div>
            }
        </div>
    );
};